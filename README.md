...
Releases are built by GitHub Actions (`.github/workflows/release.yml`) when a version
tag is pushed:
...
The same diagram always produces the same file -- places and transitions are written in
label order, arcs in source-then-destination order -- so the generated `.spn` can be kept
beside the diagram and a change to one shape shows up as a change to one line.

**The conversion is one way.** There is no `.spn` to `.drawio`, so the diagram is the
source and the generated definition is a build artefact: editing the `.spn` by hand (or
with `-pre` / `-post`) leaves the two to drift apart with nothing to detect it. `gospn
view` draws a net, but it writes DOT for viewing, not a diagram to edit.
...
Please add the shape library `MRSPN`, which is `library/MRSPN.xml` in this repository:
...
It is **not required**: `gospn gen` recognises any object carrying the right `type`
property, and the library only saves drawing them by hand. It is kept in the repository
because it is the first step of this workflow and nothing else records what it should
contain. The gist it used to be loaded from is the same file and remains as a mirror.
